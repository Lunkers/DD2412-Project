...
class NN(nn.Module):
    ...
    def forward(self, x):
        if self.use_normalization == "actnorm":
            x = F.relu(self.normalize1(self.conv1(x), if_logdet=False))
            x = F.relu(self.normalize2(self.conv2(x), if_logdet=False))
            return self.normalize3(self.conv3(x), if_logdet=False)
